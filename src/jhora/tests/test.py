...
# ============================================================
# FILE PATHS - YOUR DOWNLOADS FOLDER
# ============================================================
BASE_DIR = r"C:\LaptopBackup\Local\Personal\GitHub\JHora_World_Data"

ALLCOUNTRIES_FILE = os.path.join(BASE_DIR,"cities500.txt")# "allCountries.txt")
...
OUTPUT_FILE = os.path.join(BASE_DIR, "geonames_places_5k.csv")
...
# HELPERS
# ============================================================
def normalize_alternate_names(value, sep="|"):
    """
    ...
    remove blanks, and remove duplicates while preserving order.
    """
    seen = set()
    items = []
...
def build_output_csv(
    ...
    population_min:
      None = no population filter
      number = keep only rows with population >= number
    """
    ...
    for i, chunk in enumerate(chunk_iter, start=1):
        ...
        if population_min is not None:
            chunk = chunk[chunk["population_num"] >= population_min].copy()
        ...
        if include_alternate_names:
            ...
            chunk["alternatenames"] = chunk["alternatenames"].apply(
                lambda x: normalize_alternate_names(x, alt_name_separator)
            )
        ...
        if include_ascii_name:
            selected_cols.insert(1, "asciiname")

        if include_alternate_names:
            selected_cols.insert(2 if include_ascii_name else 1, "alternatenames")
