...
def get_dhasa_bhukthi(
    ...
    round_duration=True      # NEW: round only returned duration values; internal calcs use full precision
):
    """
        returns dasha segments at the selected depth level (L1..L6)
        ...
        @param use_tribhagi_variation: False (default), True => durations in three phases
        ...
        @param dhasa_level_index: Depth (1..6)
            ...
            5 = + Prana
            6 = + Deha-antara
        @param round_duration: If True, round only the returned duration to const.DHASA_DURATION_ROUNDING_TO
    ...
    def _recurse(level, parent_lord, parent_start_jd, parent_duration_years, prefix):
        ...
        if level < dhasa_level_index:
            ...
            # leaf rows: round only in return (if requested)
            for blord in bhukthis:
