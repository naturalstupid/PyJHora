...
def get_dhasa_bhukthi(
    ...
    round_duration=True       # NEW: affects only the returned duration values
):
    """
        returns a list of dasha segments at the selected depth level (L1..L6)
        ...
        @param dhasa_level_index: Depth (1..6)
            ...
            5 = + Prana
            6 = + Deha-antara
        @param round_duration: If True, round only the returned duration values to
                               const.DHASA_DURATION_ROUNDING_TO; internal calcs use full precision.
        ...
        @return:
            ...
            (leaf tuple includes duration; tuple grows by one lord per requested level)
    """
    ...
    def _recurse(level, parent_lord, parent_start_jd, parent_duration_years, prefix):
        ...
        child_dur_unrounded = parent_duration_years / len(bhukthis)  # equal split (same as your Antara)
        ...
        if level < dhasa_level_index:
            ...
            # leaf rows: round only for return (if requested)
            for blord in bhukthis:
    ...
    for _ in range(_dhasa_cycles):
        for _ in range(len(dhasa_adhipathi_list)):
            # Maha duration — full precision for calcs; round only in return value
