...
class VedicAnalogClock(QWidget):
    ...
    def _get_drik_info(self):
        ...
        ssvt = self.get_vedic_time(self._today_sunset)
    ...
    def drawFixedCaptions(self, painter):
        ...
        for i in range(num_captions):
            ...
            painter.restore()
    def drawHand(self, painter, centerX, centerY, length, angle, color):
        pen = QPen(color, _HAND_PEN_WIDTH)
        painter.setPen(pen)
        x = centerX + int(length * math.cos(math.radians(angle+_SUNRISE_LABEL_DEGREES)))
        y = centerY + int(length * math.sin(math.radians(angle+_SUNRISE_LABEL_DEGREES)))
...
if __name__ == "__main__":
    ...
    #jd = None; place = None
    #clock_widget = VedicDigitalClock(language='ta', jd=jd,place=place)
    clock_widget = VedicAnalogClock(language='ta', jd=jd,place=place)
    sys.exit(app.exec())
